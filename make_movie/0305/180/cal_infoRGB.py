...
DIFF_OUTPUT_DIR = "rgb_max_diff_maps"
DIFF_SCALE_TAG = "rgbmax_scale"
MAX_OFFSET = 1
...
DEFAULT_INDEXES = range(1, 20)

# CSVヘッダー定義（見やすさ重視の列構成）
CSV_FIELDS = [
    "folder",
    "avg_accuracy",
    "max_accuracy",
...
def save_rgb_max_diff_map(increased, decreased, output_path: Path):
    """cal-from-2frame-RGB-oute.py 互換の2値可視化マップを保存する。"""
    unchanged = ~(increased | decreased)
    combined = np.zeros((*increased.shape, 3), dtype=np.float32)
    combined[increased] = (0.0, 0.0, 0.0)
    combined[decreased] = (0.0, 0.0, 0.0)
    combined[unchanged] = (1.0, 1.0, 1.0)
...
def summarize_pair_metrics(folder_name: str, pair_records: List[Dict[str, float]]) -> Dict[str, float]:
    ...
    # ベストペアは Accuracy 最優先、同点時は Recall が高く Noise が低い方を優先。
    best_pair = max(
        pair_records,
        key=lambda record: (record["accuracy"], record["recall"], -record["noise"]),
    )
    ...
    return {
        ...
        "best_pair_score": best_pair["accuracy"],
    }
...
def evaluate_directory(target_dir: Path) -> Optional[Dict[str, float]]:
    ...
    diff_output_dir = target_dir / DIFF_OUTPUT_DIR
    ...
    for idx, (path1, path2) in enumerate(candidate_pairs, start=1):
        ...
        diff_filename = f"{pair_name}_{DIFF_SCALE_TAG}{QUALITY_SCALE:.2f}.png"
        diff_output_path = diff_output_dir / diff_filename
        
        save_evaluated_map(increased, decreased, roi_slices[2], (precision, recall, accuracy, fpr), output_path)
        save_rgb_max_diff_map(increased, decreased, diff_output_path)
