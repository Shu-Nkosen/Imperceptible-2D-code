...
def main():
    ...
    cache = {}
    def get_image(path):
        if path not in cache: cache[path] = load_image_as_rgb(str(path), scale=QUALITY_SCALE, method=RESIZE_METHOD)
        return cache[path]

    print(f"{len(candidate_pairs)}ペアを処理開始...")
    for i, (path1, path2) in enumerate(candidate_pairs):
        img1 = get_image(path1)
        img2 = get_image(path2)
        if img1.shape[:2] != gt_mask.shape: continue
