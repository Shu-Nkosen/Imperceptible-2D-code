...
& {
  ...
  $src  = Join-Path $root "make_movie/movie_em15/make_movie_2photo.c"
  $inc  = Join-Path $root "vcpkg/installed/x64-mingw-dynamic/include"
  $lib  = Join-Path $root "vcpkg/installed/x64-mingw-dynamic/lib"

  if (!(Test-Path $src)) { throw "src not found: $src" }
  if (!(Test-Path $inc)) { throw "include not found: $inc" }
  if (!(Test-Path $lib)) { throw "lib not found: $lib" }
