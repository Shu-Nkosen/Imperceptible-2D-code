...
def evaluate_pattern(tile_array, grid_rows, grid_cols, threshold):
    ...
    for factor_x in center_factors:
        for factor_y in center_factors:
            consistent_cells = 0
            total_cells = 0
        ...
        # 各格子の中心ピクセルの明度を取得
            for row in range(grid_rows):
                for col in range(grid_cols):
                    center_y = int((row + factor_x) * cell_height)
                    center_x = int((col + factor_y) * cell_width)
                ...
                # 現在の格子の中心明度
                    ...
                    # 四方の隣接格子の明度と比較
                        if col + 1 < grid_cols:  # 右隣
                            neighbor_x = int((col + 1 + factor_x) * cell_width)
                        ...
                        if row + 1 < grid_rows:  # 下隣
                            neighbor_y = int((row + 1 + factor_y) * cell_height)
...
for brightness in range(5, 51, 5):
    ...
    # image_path = "News20.png"

    # 実行部分
    if __name__ == "__main__":
        result = decode_brightness(image_path, brightness*0, score_threshold)
        # print("画像のデータ:", result)
